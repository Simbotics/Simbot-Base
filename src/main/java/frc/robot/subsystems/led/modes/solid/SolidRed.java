...
public class SolidRed extends LEDMode {
  ...
  public void periodic(int segmentIndex) {
    ...
    LEDColour redLedColour = new LEDColour(255, 0, 0); // Create a new RGB sequence with only red value being added

    // For every LED in the segment window, we are going to set its colour
    for (int i = minSegWindow; i < maxSegWindow; i++) {
      LEDConstants.buffer.setRGB(i, (int) redLedColour.getRed(), (int) redLedColour.getGreen(), (int) redLedColour.getBlue());
    }
  }
}
