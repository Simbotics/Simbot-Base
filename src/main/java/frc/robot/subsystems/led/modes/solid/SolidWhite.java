...
public class SolidWhite extends LEDMode {
  ...
  public void initialize() {
    System.out.println(
        "Starting the SolidWhite LED mode"); // Say we are using the WolidWhite LED mode
  }
  ...
  public void periodic(int segmentIndex) {
    ...
    for (int i = minSegWindow; i < maxSegWindow; i++) {
      LEDConstants.buffer.setRGB(
          i, whiteLedColour.getRedInt(), whiteLedColour.getGreenInt(), whiteLedColour.getBlueInt());
    }
  }
}
