...
public class PrideBi extends LEDMode {
  ...
  public void periodic(int segmentIndex) {
    ...
    // For every LED in the segment window, set its color based on the part of the segment it belongs to
    for (int i = minSegWindow; i < maxSegWindow; i++) {
      if (i < minSegWindow + segmentSize) {
        LEDConstants.buffer.setRGB(i, magentaLedColour.getRedInt(), magentaLedColour.getGreenInt(), magentaLedColour.getBlueInt());
      } else if (i < minSegWindow + 2 * segmentSize) {
        LEDConstants.buffer.setRGB(i, purpleLedColour.getRedInt(), purpleLedColour.getGreenInt(), purpleLedColour.getBlueInt());
      } else {
        LEDConstants.buffer.setRGB(i, royalBlueLedColour.getRedInt(), royalBlueLedColour.getGreenInt(), royalBlueLedColour.getBlueInt());
      }
    }
  }
}
