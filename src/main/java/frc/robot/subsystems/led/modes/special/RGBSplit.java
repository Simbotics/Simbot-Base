  ...
  public void periodic(int segmentIndex) {
    ...
    for (int i = minSegWindow; i < maxSegWindow; i++) {
      if (i < minSegWindow + segmentSize) {
        LEDConstants.buffer.setRGB(i, redLedColour.getRedInt(), redLedColour.getGreenInt(), redLedColour.getBlueInt());
      } else if (i < minSegWindow + 2 * segmentSize) {
        LEDConstants.buffer.setRGB(i, greenLedColour.getRedInt(), greenLedColour.getGreenInt(), greenLedColour.getBlueInt());
      } else {
        LEDConstants.buffer.setRGB(i, blueLedColour.getRedInt(), blueLedColour.getGreenInt(), blueLedColour.getBlueInt());
      }
    }
  }
}
